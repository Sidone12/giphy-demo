...
<div class="d-flex justify-center" v-if="giphsStore.giphs.length > 0">
    <v-card max-width="600" width="600">
        <v-card-text>
            ...
            <div class="text--primary">
                Profile: <a target="_blank" :href="`http://${currentUser?.user.profile_url}`">{{
                        currentUser?.user.profile_url
                    }}</a>
            </div>
            <p>instagram: <a target="_blank" :href="`http://${currentUser?.user.instagram_url}`">{{
                    currentUser?.user.instagram_url
                }}</a></p>
        </v-card-text>
        <v-card-actions>
